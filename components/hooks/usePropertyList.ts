...
export function usePropertyList(
  ...
  useEffect(() => {
    ...
    if (propertyFilter.propertyType) {
      ...
      result = result.filter(
        (property) => property.type?.trim().toLowerCase() === type
      );
    }
    // City
    if (propertyFilter.city) {
      ...
      result = result.filter(
        (property) => property.city?.trim().toLowerCase() === city
      );
    }
    // State
    if (propertyFilter.state) {
      ...
      result = result.filter(
        (property) => property.state?.trim().toLowerCase() === state
      );
    }
    // Status
    if (propertyFilter.status) {
      ...
      result = result.filter(
        (property) => property.status?.trim().toLowerCase() === status
      );
    }
    // Amenities
      ...
      result = result.filter((property) =>
        propertyFilter.amenities.every((amenity: string) =>
          property.amenities.some(
            (a) => a.trim().toLowerCase() === amenity.trim().toLowerCase()
          )
        )
      );
    }
    if (propertyFilter.bedrooms?.min > 0) {
      result = result.filter(
        (property) => property.bedrooms >= propertyFilter.bedrooms.min
      );
    }
    if (propertyFilter.bathrooms?.min > 0) {
      result = result.filter(
        (property) => property.bathrooms >= propertyFilter.bathrooms.min
      );
    }
    if (propertyFilter.garages?.min > 0) {
      result = result.filter(
        (property) => property.garages >= propertyFilter.garages.min
      );
    }
    if (propertyFilter.rooms?.min > 0) {
      result = result.filter(
        (property) => property.rooms >= propertyFilter.rooms.min
      );
    }
    ...
    const isSame =
      ...
      result.every(
        (item, idx) => item.id === state.filteredProperties[idx]?.id
      );
    if (!isSame) {
  ...
  const totalPages = Math.ceil(
    state.filteredProperties.length / state.itemsPerPage
  );
